...
const RECEIVER_FOOTPRINT_PADDING = 1.18;
...
function collectArchitecture(scene) {
  ...
  if (!box.isEmpty()) {
    ...
    // Architectural contact plane, NOT the visually lowered site slab.
    groundY = box.min.y + state.receiverOffset;
  }
  frustumDirty = true;
}
...
function makeReceiver(scene) {
  ...
  const width = Math.max(0.5, size.x * RECEIVER_FOOTPRINT_PADDING);
  const depth = Math.max(0.5, size.z * RECEIVER_FOOTPRINT_PADDING);
  ...
  receiver.userData.adamShadowGroundY = groundY;
...
function updateReceiver() {
  if (!receiver || !sceneBounds || sceneBounds.isEmpty()) return;
  groundY = sceneBounds.min.y + state.receiverOffset;
  receiver.position.set(contentCentre.x, groundY, contentCentre.z);
  receiver.userData.adamShadowGroundY = groundY;
...
function makeShadowLight(scene, renderer) {
  ...
  state.mapSize = Math.min(state.mapSize, maxTexture);
...
function fitShadowCamera() {
  ...
  fittedFrustum={width:cam.right-cam.left,height:cam.top-cam.bottom,near:cam.near,far:cam.far};
  frustumDirty=false;
...
function updateStatus() {
  ...
  const fit=fittedFrustum?` · fitted ${fittedFrustum.width.toFixed(1)}×${fittedFrustum.height.toFixed(1)}`:'';
  statusEl.textContent=state.enabled
    ? `shadow V6 CONTACT PLANE · hooks ${renderTicks} · ${architecture.length} casters · ground Y ${groundY.toFixed(3)} · VSM ${state.mapSize}px × ${VSM_BLUR_SAMPLES} blur${fit} · map ${shadowLight?.shadow?.map?'READY':'pending'}`
...
function install(renderer,scene) {
  ...
  console.info('[ADAM shadows V6 contact plane]',{
