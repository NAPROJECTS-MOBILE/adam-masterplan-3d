/* ADAM Spline motion v5.1 — 53 visible targets.
   Fix: cluster-4 Rectangle_5 is co-located with mesh_9_instance_5.
   The user could only pick one of that overlapping pair. The full 10-child
   replica parent now moves with Rectangle_5 so the hidden duplicate never
   gets exposed as a stationary block. */
...
export function createSplineMotion(model,opts={}){
 ...
 if(debug){
  console.group('[spline-motion v5.1]');
 ...
 function setAmbientTime(seconds){
  for(const b of amb){
   ...
   if(d.p)o.position.set(d.p[0]+d.dp[0]*f*unitScale,d.p[1]+d.dp[1]*f*unitScale,d.p[2]+d.dp[2]*f*unitScale);
